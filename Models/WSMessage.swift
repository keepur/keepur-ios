...
enum WSIncoming {
    ...
    static func decode(from data: Data) -> WSIncoming? {
        ...
        case "session_list":
            guard let sessionsArr = json["sessions"] as? [[String: Any]] else { return nil }
            let sessions = sessionsArr.compactMap { dict -> ServerSession? in
        ...
        case "browse_result":
            guard let path = json["path"] as? String,
                  let entriesArr = json["entries"] as? [[String: Any]] else { return nil }
            let entries = entriesArr.compactMap { dict -> BrowseEntry? in
