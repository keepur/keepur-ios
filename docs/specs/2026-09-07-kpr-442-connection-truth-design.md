...
- **Team at-least-once**: an un-acked message whose ack was lost to a disconnect is re-sent and may be duplicated server-side. Accepted by the epic ("re-sends rather than staying 'sending' forever"); E's `serverId` reconcile is the dedup story. Queued text and retained attachments are in-memory on both layers; an app kill before reconnect loses the queues and retained attachments, while persisted message rows remain without restoring or replaying either queue.
