...
- **Risk**: until ticket zero merges, a PR merges on the manual quality gate only. Ticket zero is deliberately minimal (build + unit tests on one simulator destination with default ad-hoc simulator signing, no certificates or provisioning, no UI tests) so it cannot itself become a project.
