...
- The socket waits for `URLSessionWebSocketDelegate.didOpen` before its opening round trip. Beekeeper uses its JSON ping/pong so proxy-held control pings cannot stall startup, buffering earlier non-pong frames for normal delivery; Hive uses a WebSocket control ping because its protocol ignores JSON ping. After the handshake, the socket starts receiving and sends the bare application ping before `onConnected` bookkeeping; every 30 seconds it sends both the application ping and a control ping. Control-ping failures use generation-checked reconnect/auth handling.
...
- A ready concierge recovers from its session-scoped “No conversation found with session ID” error, including persisted system-error rows checked on entering ready. Recovery uses `requestSessionClear(sessionId:)` without eager local deletion, waits up to 3 seconds for the matching `session_cleared`, then clears cache/registered identity and waits up to 5 seconds for a fresh concierge spawn. A missing clear acknowledgement leaves the cache intact and does not spawn; the same run cancellation and reply-buffer rules apply.
...
Use main-actor observable VMs, guard optional values, existing MARK/file organization and theme tokens. E's optional serverId is the only stored-schema addition; keep all other storage boundaries, the five-model container and shared reporting helper. Test deterministic behavior with real in-memory SwiftData plus injected operation attempts and fake transport, never by forcing invalid containers or treating unique upserts as errors.
