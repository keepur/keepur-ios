...
- The socket waits for `URLSessionWebSocketDelegate.didOpen` before its opening round trip. Beekeeper uses its JSON ping/pong so proxy-held control pings cannot stall startup; Hive uses a WebSocket control ping because its protocol ignores JSON ping. After the handshake, the socket starts receiving and sends the bare application ping before `onConnected` bookkeeping; the application ping then repeats every 30 seconds.
