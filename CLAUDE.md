...
- **Headless Chat core**: `ChatViewModel` accepts injected socket, credentials, optional speech, and `Duration` timeouts. `speechManager` retains the supplied instance or creates it lazily; configure, frame handling, queues, and sends with read-aloud off must not construct speech.
...
- **Tool approvals**: Modal sheet with 60s countdown, auto-deny on timeout. Pending approvals are keyed by owning session; another session's approval never changes selection or path, and approve/deny removes only the addressed entry.
